...
private:
    ...
    static bool barycentric2D(float px, float py, float ax, float ay, float bx, float by,
...
public:
    static std::vector<float> generate(const stl& mesh, int width, int height,
        const Camera& cam, float ortho_scale,
        float& out_zmin, float& out_zmax,
        float depth_near, float depth_far,
        float bg_separation,
        bool addFloor = true,
        float floorHeightRatio = 0.1f)   // 10% of image height
    {
...
private:
    static void processTriangle(const float* tri, const Camera& cam,
        ...
        int width, int height, std::vector<float>& zbuffer)
    {
        ...
        for (int y = miny; y <= maxy; ++y) {
            for (int x = minx; x <= maxx; ++x) {
                processPixel(x, y, px, py, zcam, width, zbuffer);
            }
        }
    }

    static void processPixel(int x, int y, const float px[3], const float py[3],
        const float zcam[3], int width, std::vector<float>& zbuffer)
    {
        float u, v, w;
        if (!barycentric2D(x + 0.5f, y + 0.5f, px[0], py[0], px[1], py[1], px[2], py[2], u, v, w))
            return;
        if (u < 0 || v < 0 || w < 0) return;

        float z_interp = u * zcam[0] + v * zcam[1] + w * zcam[2];
        if (z_interp <= 0.0f) return;
    ...
    static std::vector<float> finalizeDepthMap(const std::vector<float>& zbuffer, int width, int height,
        float& out_zmin, float& out_zmax,
        float depth_near, float depth_far,
        float bg_separation)
    {
...
class DepthMapGenerator_original {
